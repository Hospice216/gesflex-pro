...
SELECT 
    ...
    CASE 
        WHEN proname = 'is_superadmin' THEN 
            CASE WHEN is_superadmin() THEN 'true' ELSE 'false' END
        WHEN proname = 'is_admin' THEN 
            CASE WHEN is_admin() THEN 'true' ELSE 'false' END
        WHEN proname = 'get_current_user_id' THEN 
            get_current_user_id()::TEXT
        ELSE 'N/A'
    END as function_result,
    CASE 
        WHEN proname IN ('is_superadmin', 'is_admin', 'get_current_user_id') THEN '✅ EXISTE'
        ELSE '❌ MANQUANTE'
    END as status
FROM pg_proc 
...
SELECT 
    ...
    CASE WHEN is_superadmin() THEN 'true' ELSE 'false' END as result,
    CASE 
...
SELECT 
    ...
    CASE WHEN is_admin() THEN 'true' ELSE 'false' END as result,
    CASE 
